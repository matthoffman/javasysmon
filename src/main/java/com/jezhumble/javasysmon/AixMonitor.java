...
class AixMonitor implements Monitor {
    private static Monitor monitor = null;
    private final FileUtils fileUtils;

    static {
        if (System.getProperty("os.name").toLowerCase().startsWith("aix")) {
            if (System.getProperty("os.arch").toLowerCase().startsWith("ppc")) {
                new NativeLibraryLoader().loadLibrary("javasysmonaix.so");
                monitor = new AixMonitor();
            }
        }
    }

    public AixMonitor() {
        JavaSysMon.addSupportedConfig("AIX");
        if (monitor != null) {
